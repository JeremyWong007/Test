...
function createTafAccount(){
    ...
    keys=`cleos new key --to-console | awk '{print $3}'`
    private_key=`echo $keys | awk '{print $1}'`
    public_key=`echo $keys | awk '{print $2}'`
    account=`cleos new account $public_key | grep "account address:"`
    if [ "$account" == "" ]; then
    	...
    	cleos wallet importkey --private-key $private_key
    	#echo $account ${accountsAndKeys[$account]}

    	cleos transfer T17Y2vaQtyQmGVm3MZrvS3hGFj5zVXvo48S $account "1 EOS"
    	cleos producer delegatesource $account $account "0.1 EOS" "0.1 EOS"
    fi
}
...
function delegatesource(){
    ...
    for acc in ${!accountsAndKeys[@]}; do
        cleos producer delegatesource $acc $acc "0.1 EOS" "0.1 EOS"
    done
}
...
function test_transfer(){
    ...
    do
        ...
        cleos transfer $accountFrom $accountTo "$asset EOS"
    done
}
...
function importAccountskey(){
    ...
    for keys in ${accountsAndKeys[@]}; do
        echo ${keys%/*}
    	cleos wallet importkey --private-key ${keys%/*}
    done
}
...
#cleos producer delegatesource T1BvMyqRo9BnHKi1DamjF1MwWcGT3EGfrCA T17pFoCekTq9Ukui6jgFhu2QLc9F5tMvJif "500 EOS" "500 EOS"
function init(){
...
function createTafAccount2(){
    ...
    if [ "$account" == "" ]; then
    	...
    	cleos transfer T17Y2vaQtyQmGVm3MZrvS3hGFj5zVXvo48S $account "10000 EOS"
    	cleos producer delegatesource $account $account "40 EOS" "40 EOS"
    fi
}
...
function giveMoreBalance(){
    ...
    for acc in ${accountVector[@]}; do
        cleos transfer T17Y2vaQtyQmGVm3MZrvS3hGFj5zVXvo48S $acc "9900 EOS"
    done
...
function stakeAndVote(){
    ...
    do
        ...
        cleos producer delegatesource $NAME $NAME "$ASSET EOS" "0 EOS"
