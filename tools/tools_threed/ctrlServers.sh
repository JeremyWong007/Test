...
function killAllTaf(){
    ...
    do
        ...
        #cmd="ps -e | grep eos | awk '{print \$1}' |xargs kill -9"
        cmd="pkill nodeos"
...
function reboot140(){
    cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChain.sh $EOS_MANAGE_VERSION"
    #cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr local140 --clean $EOS_MANAGE_VERSION"
    ctrolLocal140
    cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr pubYmx --clean $EOS_MANAGE_VERSION"
    ctrolYamaxun
    cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr pubAli --clean $EOS_MANAGE_VERSION"
    ctrolAli
}
...
function checkHost(){
    ...
    do
        ...
        cmd="ps -e | grep nodeos | awk '{print \$4}'"
        result=`sshpass -p "Yongqi@123" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 $username@$host $cmd`
        if [ "$result" == "nodeos" ]; then
            echo "$host nodeos is running"
            cmd="cleos query bcinfo | grep \"last_irreversible_block_num\""
            result=`sshpass -p "Yongqi@123" ssh  -o StrictHostKeyChecking=no -o ConnectTimeout=10 $username@$host $cmd`
            echo "$result"
        else
            echo "[ERROR!!]$host nodeos is not running"
        fi
    done
}
...
function reboot140NotRun(){
    ...
    do
        ...
        if [ "$result" == "nodeos" ]; then
            ...
            #echo "[ERROR!!]  nodeos is not running"
            cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr pubYmx --clean"
            result=`ssh -i /data/info/tafnode.pem -o StrictHostKeyChecking=no -o ConnectTimeout=10 $username@$host $cmd`
        
            cmd="ps -e | grep nodeos | awk '{print \$4}'"
            result=`ssh -i /data/info/tafnode.pem -o StrictHostKeyChecking=no -o ConnectTimeout=10 $username@$host $cmd`
            if [ "$result" == "nodeos" ]; then
                echo "$host eos chain reboot ok"
            else
                echo "[ERROR!!] $host eos chain reboot error"
            fi
        fi
    done
    ...
    do
        ...
        if [ "$result" == "nodeos" ]; then
            ...
            #echo "[ERROR!!]  nodeos is not running"
            cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChain.sh"
            result=`sshpass -p "Yongqi@123" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 $username@$host $cmd`
...
function ctrl140After16NotRun(){
    ...
    do
    {
        ...
        num=`expr ${LOCAL_IP_BASE} + $i`
        host="192.168.101.$num"
        cmdIn="ps -e | grep nodeos | awk '{print \$4}'"
...
function setCmd(){
    #cmd="echo \"EOS_NODE_NUM=$i\" > /data/info/env.eos;cat /data/info/env.eos"
    #cmd="echo \"EOS_NODE_NUM=${AliHostNum[$hostname]}\" > /data/info/env.eos;cat /data/info/env.eos"
    #cmd="echo \"EOS_NODE_NUM=${YamaxunHostNum[$hostname]}\" > /data/info/env.eos;cat /data/info/env.eos"
    #cmd="echo \"EOS_NODE_NUM=${Local1[$hostname]}\" > /data/info/env.eos;cat /data/info/env.eos"
    #cmd="echo \"EOS_NODE_NUM=${LocalHostV2Num[$hostname]}\" > /data/info/env.eos;cat /data/info/env.eos"
    :
}
#cmd="ps -e | grep eos | awk '{print \$1}' |xargs kill -9"
#cmd="pkill nodeos; pkill tafwalletd"
#cmd="pkill tafwalletd"
#cmd="ps -e | grep eos"
#cmd="ulimit -s | grep 1"
#cmd="rm ~/eosio-wallet/default.wallet"
#cmd="cd /data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/tools;./unlock.sh"
#cmd="ulimit -s 102400; /data/info/test/test"
#cmd="sudo mkdir -p /data/info/file;sudo chmod 777 -R /data"
#cmd="scp -r -i /data/info/tafnode.pem  -o StrictHostKeyChecking=no -o ConnectTimeout=10 ubuntu@172.31.21.227:/data/info/file/release_${EOS_MANAGE_VERSION}/ /data/info/file/"
#cmd="sudo dpkg -r eosio;sudo dpkg -i /data/info/file/release_${EOS_MANAGE_VERSION}/tafSoftware_0.0.1.deb"
#cmd="echo Yongqi@123 | sudo -S dpkg -r eosio;sudo dpkg -i /data/info/file/release_${EOS_MANAGE_VERSION}/tafSoftware_0.0.1.deb"
#cmd="ps -e | grep nodeos | awk '{print \$4}'"
#cmd="rm -rf /data/info/file/release_7.21"
#cmd="rm -rf /data/biosboot/pubchain"
#cmd="apt install sshpass"
##cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr v3 --clean"
#cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr v3"
#cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr v3 --hardReplay"
#cmd="ulimit -s 102400; cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr v3"
#cmd="ulimit -s 102400; cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr v4"
#cmd="ulimit -s 102400; cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr v5"
##cmd="rm -rf /data/biosboot/pubchain"
#cmd="cd /data/info/file/release_10.14/biosboot/tools;./rebootReleaseChain.sh"
##cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr local140 --clean"
##cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr localv2 --clean"
#cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr localv2"
#ctrolLocal140
#ctrolLocal140Bx
#ctrolLocalFirst16
#ctrolLocalFirst16Bx
...
#ctrolLocalOne
##cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr pubYmx --clean"
##cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr pubv2 --clean"
#cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr pubv2"
#ctrolYamaxun
#ctrolYamaxunBx
#ctrolYamaxunBxPart1
#ctrolYamaxunBxPart2
##cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr pubAli --clean"
##cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr pubv2 --clean"
#cmd="cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChainNew.sh --p2pAddr pubv2"
#ctrolAli
#ctrolAliBx
...
#cmd="source /etc/profile; cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChain.sh --p2pAddr v1"
#cmd="source /etc/profile; cd /data/info/file/release_$EOS_MANAGE_VERSION/biosboot/tools;./rebootReleaseChain.sh --p2pAddr v1 --clean"
#ctrolLocal1Bx
...
function copyLocalToRelease(){
    ...
    COPY_FILE_DEST="/data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/tools/"
    #COPY_FILE_DEST="/data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/"
    cp -rf $COPY_FILE_SRC_LOCAL $COPY_FILE_DEST #注意，源路径需与目标路径一致
}
#copyLocalToRelease

#COPY_FILE_SRC="/data/info/file/release_${EOS_MANAGE_VERSION}/"
#COPY_FILE_DEST="/data/info/file/"
#COPY_FILE_SRC="/etc/profile"
#COPY_FILE_DEST="/etc/"
#COPY_FILE_SRC="/data/info/file/release_${EOS_MANAGE_VERSION}/contracts"
#COPY_FILE_SRC="/data/info/file/release_${EOS_MANAGE_VERSION}/test/*"
#COPY_FILE_DEST="/data/info/file/release_${EOS_MANAGE_VERSION}/test/"
#COPY_FILE_SRC="/data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/tools/setConfig.sh"
#COPY_FILE_SRC="/data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/tools/rebootReleaseChainNew.sh"
#COPY_FILE_SRC="/data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/tools/rebootReleaseChain.sh"
#COPY_FILE_SRC="/data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/tools/ctrlChain.sh"
#COPY_FILE_SRC="/data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/tools/*"
#COPY_FILE_SRC="/data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/genesis3.json"
#COPY_FILE_DEST="/data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/tools/"
#COPY_FILE_DEST="/data/info/file/release_${EOS_MANAGE_VERSION}/biosboot/"
